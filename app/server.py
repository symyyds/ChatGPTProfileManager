...
def codex_executable() -> str:
    candidates = [
        r"C:\Users\26895\AppData\Roaming\npm\node_modules\@openai\codex\node_modules\@openai\codex-win32-x64\vendor\x86_64-pc-windows-msvc\codex\codex.exe",
        r"C:\Program Files\WindowsApps\OpenAI.Codex_26.602.4764.0_x64__2p2nqsd0c76g0\app\resources\codex.exe",
    ]
    for candidate in candidates:
        if Path(candidate).exists():
            return candidate
    for path in os.environ.get("PATH", "").split(os.pathsep):
        candidate = Path(path) / "codex.exe"
        if candidate.exists():
            return str(candidate)
...
def main() -> None:
    ...
    print(f"ChatGPT Profile Manager: http://127.0.0.1:{port}", flush=True)
